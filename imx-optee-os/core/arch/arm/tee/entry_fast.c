...
static void tee_entry_get_shm_config(struct thread_smc_args *args)
{
	...
	// tzc_configure_region(4, (CFG_TEE_CONTAINER_START - DRAM0_BASE),TZC_ATTR_REGION_SIZE(TZC_REGION_SIZE_32M) | TZC_ATTR_REGION_EN_MASK | TZC_ATTR_SP_S_RW | TZC_ATTR_SP_NS_RO);
	// tzc_configure_region(7, (CFG_TEE_CONTAINER_START - DRAM0_BASE + SZ_32M),TZC_ATTR_REGION_SIZE(TZC_REGION_SIZE_32M) | TZC_ATTR_REGION_EN_MASK | TZC_ATTR_SP_S_RW | TZC_ATTR_SP_NS_RO);
	// tzc_dump_state();
#endif
	DMSG("tee_container_vaddr: %#lx, default_nsec_shm_paddr: %#lx, default_nsec_shm_size %#lx", tee_container_vaddr, default_nsec_shm_paddr, default_nsec_shm_size);
}
...
{
	entry_count++;
	num = OPTEE_SMC_FUNC_NUM(args->a0);
	// if(args->a1 < UL(0xfa000000))
	// if(entry_count % 10 == 0)
	// 	DMSG("ENTRY %d optee args->a0: %d, args->a1 %#lx", entry_count, OPTEE_SMC_FUNC_NUM(args->a0), args->a1);
	switch (args->a0) {
	...
	case OPTEE_SMC_GET_SHM_CONFIG:
		...
		break;
	// add start by xsyin
	case OPTEE_SMC_INIT_CONTAINER_REGION:
		tee_entry_init_container_region(args);
		break;
	case OPTEE_SMC_CONTAINER_REGION_UPDATE:
		tee_entry_update_container_region(args);
		break;
	case OPTEE_SMC_CONTAINER_REGION_MEMCPY:
		tee_entry_memcpy_container_region(args);
		break;
	case OPTEE_SMC_CONTAINER_DATA_INIT:
		tee_entry_container_data_init(args);
		break;
	case OPTEE_SMC_CONTAINER_DATA_DESTORY:
		tee_entry_container_data_destory(args);
		break;
	case OPTEE_SMC_UINT64_ATOMIC_DEC:
		tee_entry_uint64_dec(args);
		break;
	case OPTEE_SMC_UINT64_ATOMIC_INC:
		tee_entry_uint64_inc(args);
		break;
	// add end by xsyin
	...
	default:
		DMSG("args->a0 %#lx, a1 %#lx", args->a0, args->a1);
		args->a0 = OPTEE_SMC_RETURN_UNKNOWN_FUNCTION;
		break;
	}
	// DMSG("ENTRY %d optee args->a0: %d, args->a1 %#lx", entry_count, OPTEE_SMC_FUNC_NUM(args->a0), args->a1);
