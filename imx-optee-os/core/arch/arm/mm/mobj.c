...
static bool mobj_phys_matches(struct mobj *mobj, enum buf_is_attr attr)
{
	...
	case CORE_MEM_NON_SEC:
		return a == CORE_MEM_NSEC_SHM || a == CORE_MEM_TEE_CONTAINER;
	case CORE_MEM_TEE_CONTAINER:
			     ...
			     enum buf_is_attr battr)
{
	...
	case CORE_MEM_NSEC_SHM:
		...
		break;
	case CORE_MEM_TEE_CONTAINER:
		area_type = MEM_AREA_TEE_CONTAINER;
		break;
	case CORE_MEM_SDP_MEM:
				...
				paddr_t page_offset, uint64_t cookie)
{
	...
	if (!mobj_reg_shm){
		DMSG("mobj_reg_shm_alloc, mobj_reg_shm!!!");
		return NULL;
	}
	...
	for (i = 0; i < num_pages; i++) {
		if (mobj_reg_shm->pages[i] & SMALL_PAGE_MASK){
			DMSG("mobj_reg_shm_alloc, i %d, mobj_reg_shm->pages[i]: %#llx", i, mobj_reg_shm->pages[i]);
			goto err;
		}
		...
		if (!core_pbuf_is(CORE_MEM_NON_SEC, mobj_reg_shm->pages[i],
					SMALL_PAGE_SIZE)){
			DMSG("mobj_reg_shm_alloc, core_pbuf_is, mobj_reg_shm->pages[i]: %#llx", mobj_reg_shm->pages[i]);
			goto err;
		}
	}
...
static TEE_Result mobj_mapped_shm_init(void)
{
	...
	// if (!tee_mm_init(&tee_mm_con_tsk, phys_to_virt(CONTAINER_REGION_TSK_START, MEM_AREA_TEE_CONTAINER), phys_to_virt(CONTAINER_REGION_TSK_END, MEM_AREA_TEE_CONTAINER), SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
	if (!tee_mm_init(&tee_mm_con_tsk, CONTAINER_REGION_TSK_START, CONTAINER_REGION_TSK_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create tsk pool");
	if (!tee_mm_init(&tee_mm_con_cred, CONTAINER_REGION_CRED_START, CONTAINER_REGION_CRED_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create cred pool");
	if (!tee_mm_init(&tee_mm_con_nsp, CONTAINER_REGION_NSPROXY_START, CONTAINER_REGION_NSPROXY_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create nsproxy pool");
	if (!tee_mm_init(&tee_mm_con_user_ns,CONTAINER_REGION_USER_NS_START, CONTAINER_REGION_USER_NS_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create user ns pool");
	if (!tee_mm_init(&tee_mm_con_mnt_ns, CONTAINER_REGION_MNT_NS_START, CONTAINER_REGION_MNT_NS_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create mnt ns pool");
	if (!tee_mm_init(&tee_mm_con_fs, CONTAINER_REGION_FS_START, CONTAINER_REGION_FS_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create fs pool");
	if (!tee_mm_init(&tee_mm_con_mount, CONTAINER_REGION_MOUNT_START, CONTAINER_REGION_MOUNT_END, SMALL_PAGE_SHIFT, TEE_MM_POOL_NO_FLAGS))
		panic("Could not create mount pool");
