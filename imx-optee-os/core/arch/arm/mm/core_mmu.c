...
unsigned long tee_container_vaddr;
...
static void dump_mmap_table(struct tee_mmap_region *memory_map)
{
	...
	for (map = memory_map; !core_mmap_is_end_of_table(map); map++) {
		...
		if(map->type == MEM_AREA_TEE_CONTAINER )
			tee_container_vaddr = vstart;
	}
}
...
bool core_mmu_add_mapping(enum teecore_memtypes type, paddr_t addr, size_t len)
{
	...
	if (map->size < l){
		EMSG("3333 ADDR %#lx, len %#lx, l %#lx, map->size %#lx", addr, len, l, map->size);
		return false;
	}
	...
	if(type == MEM_AREA_NSEC_SHM)
		DMSG("len %#lx, L %#lx, map->va %#lx, map->size %#lx", len, l, map->va, map->size);
	map->type = type;
