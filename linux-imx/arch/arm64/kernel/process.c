...
#ifdef CON_CRED
	if(in_container_range(prev->cred, CRED))
		prev->cred = (struct cred *)mask_con_value(CRED, prev->cred);
#endif
#ifdef CON_FS
	if(in_container_range(prev->fs, FS))
		prev->fs = (struct fs_struct *)mask_con_value(FS, prev->fs);
#endif
#endif

#ifdef COUNT_SWITCH_INTERVAL
		switch_count_a++;
		if((switch_count_a >=1200000) && (switch_count_a % INTERVAL == 0)){
			u64 now = ktime_get_ns();
			pr_alert("%s: switch_count_a %ld, duration %ld, cpu %d, prev %s %#lx, cpu %d; next %s %#lx cpu %d", __func__, switch_count_a, now - switch_time_a, smp_processor_id(),prev->comm, prev, prev->cpu, next->comm, next, next->cpu);
			switch_time_a = now;
		}
