		...
		struct compat_rusage __user *, uru)
{
	...
	if (!access_ok(VERIFY_WRITE, infop, sizeof(*infop)))
		return -EFAULT;
#endif
	// if(current->cred->uid.val)
		pr_alert("%s: 1111 current %s, nsproxy %#lx, &infop->si_signo %#lx, cred %#lx, uid %d", __func__, current->comm, current->nsproxy, &infop->si_signo,current->cred, current->cred->uid.val);
	user_access_begin();
	unsafe_put_user(signo, &infop->si_signo, Efault);
	unsafe_put_user(0, &infop->si_errno, Efault);
	unsafe_put_user(info.cause, &infop->si_code, Efault);
	...
	user_access_end();
	// if(current->cred->uid.val)
		pr_alert("%s: 2222 current %s, nsproxy %#lx, &infop->si_signo %#lx, cred %#lx, uid %d", __func__, current->comm, current->nsproxy, &infop->si_signo, current->cred, current->cred->uid.val);
	return err;
Efault:
	user_access_end();
	return -EFAULT;
