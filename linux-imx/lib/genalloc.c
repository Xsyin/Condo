...
void gen_pool_free(struct gen_pool *pool, unsigned long addr, size_t size)
{
	...
	list_for_each_entry_rcu(chunk, &pool->chunks, next_chunk) {
		if (addr >= chunk->start_addr && addr <= chunk->end_addr) {
			...
			// if(remain){
			// 	struct container_cred *con_cred = (struct container_cred *)addr;
			// 	pr_alert("%s: addr %#lx, size %#lx, paddr %#lx, usage %d, cap %#lx, remain %d", __func__, addr, size, con_cred->paddr, atomic_read(&(con_cred->k_cred.usage)), con_cred->k_cred.cap_effective, remain);
			// }
			// BUG_ON(remain);
			size = nbits << order;
