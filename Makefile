...
.PHONY: imx-mkimage
imx-mkimage: optee-os imx-atf uboot-imx linux-common
...
.PHONY: imx-atf
imx-atf: optee-os
	$(MAKE) -j4 -C $(ATF_PATH) $(ARMATF_OPTEE_FLAGS) clean 
	$(MAKE) -j4 -C $(ATF_PATH) $(ARMATF_OPTEE_FLAGS) bl31 
	cp $(ATF_PATH)/build/imx8mq/$(TF_A_BUILD)/bl31.bin $(MKIMAGE_PATH)/iMX8M/
...
.PHONY: hello-module
hello-module:
	$(MAKE) -C $(LINUX_PATH) $(LINUX_MODULES_FLAGS) M=$(CURDIR) modules

obj-m += firewall.o

.PHONY: firewall-module
firewall-module:
	$(MAKE) -C $(LINUX_PATH) $(LINUX_MODULES_FLAGS) M=$(CURDIR) modules

.PHONY: linux-imx
linux-imx: linux-common
	scp $(LINUX_PATH)/arch/arm64/boot/Image root@10.42.0.156:/mnt
...
linux-module:
	...
	rm -rf $(OUT_PATH)/lib/modules/4.19.35/build $(OUT_PATH)/lib/modules/4.19.35/source
	scp -r $(OUT_PATH)/lib root@10.42.0.156:/
...
linux-common: linux-defconfig
	$(MAKE) -j12 -C $(LINUX_PATH) $(LINUX_COMMON_FLAGS)
...
optee-os: optee-os-common
	...
	cp $(OPTEE_OS_PATH)/out/arm-plat-imx/core/tee.bin $(OUT_PATH)
